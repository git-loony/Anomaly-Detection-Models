# full_billing_anomaly_pipeline_db.py
...
isolation_models = {}  # store Isolation Forest per cluster
for cluster_id in range(n_clusters):
    idx = np.where(clusters == cluster_id)[0]
    X_cluster = X_scaled[idx]
    iso = IsolationForest(contamination=0.05, random_state=42)
    iso.fit(X_cluster)
    if_scores[idx] = -iso.score_samples(X_cluster)  # higher = more anomalous
    isolation_models[cluster_id] = iso  # save model for later use
...
# Save to SQLite DB inside MODEL folder
...
print("Anomaly detection pipeline completed. All models and DB are in the MODEL folder.")